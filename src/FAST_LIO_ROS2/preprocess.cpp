...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  else
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      float r2 = src.x * src.x + src.y * src.y + src.z * src.z;
      if (r2 <= (blind * blind))
        continue;
      ...
      pt.curvature = src.offset_time / float(1000000); 

      const auto &prev = msg->points[i - 1];
      if (fabs(src.x - prev.x) < 1e-7 &&
          fabs(src.y - prev.y) < 1e-7 &&
          fabs(src.z - prev.z) < 1e-7)
      {
        continue;
      }
