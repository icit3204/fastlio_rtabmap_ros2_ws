...
#include "phase4_lifecycle_manager_reconciled/lifecycle_manager.hpp"
...
namespace phase4_lifecycle_manager_reconciled
{
...
: Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  declare_parameter("service_call_timeout_sec", 1.0);
  declare_parameter("transition_retry_interval_ms", 100);
  declare_parameter("transition_retries", 3);
  ...
  const auto service_timeout_sec = get_parameter("service_call_timeout_sec").as_double();
  const auto retry_interval_ms = get_parameter("transition_retry_interval_ms").as_int();
  transition_retries_ = get_parameter("transition_retries").as_int();
  if (service_timeout_sec <= 0.0 || retry_interval_ms < 0 || transition_retries_ < 1) {
    throw std::invalid_argument("invalid lifecycle transition recovery parameters");
  }
  service_call_timeout_ = std::chrono::seconds(
    static_cast<int64_t>(std::ceil(service_timeout_sec)));
  transition_retry_interval_ = std::chrono::milliseconds(retry_interval_ms);
...
LifecycleManager::changeStateForNode(const std::string & node_name, std::uint8_t transition)
{
  ...
  for (int attempt = 1; attempt <= transition_retries_; ++attempt) {
    uint8_t observed_state = State::PRIMARY_STATE_UNKNOWN;
    try {
      observed_state = node_map_[node_name]->get_state(service_call_timeout_);
      if (observed_state == target_state) {
        if (transition == Transition::TRANSITION_ACTIVATE) {
          return createBondConnection(node_name);
        }
        if (transition == Transition::TRANSITION_DEACTIVATE) {
          bond_map_.erase(node_name);
        }
        return true;
      }
      uint8_t expected_source_state = State::PRIMARY_STATE_UNKNOWN;
      switch (transition) {
        case Transition::TRANSITION_CONFIGURE:
          expected_source_state = State::PRIMARY_STATE_UNCONFIGURED;
          break;
        case Transition::TRANSITION_ACTIVATE:
          expected_source_state = State::PRIMARY_STATE_INACTIVE;
          break;
        case Transition::TRANSITION_DEACTIVATE:
          expected_source_state = State::PRIMARY_STATE_ACTIVE;
          break;
      ...
      const bool expected_source = observed_state == expected_source_state;
      if (!expected_source) {
        RCLCPP_ERROR(get_logger(),
          "Lifecycle state %u for %s is not a valid source for transition %u",
          observed_state, node_name.c_str(), transition);
        return false;
      }
    } catch (const std::runtime_error & error) {
      RCLCPP_WARN(get_logger(),
        "Lifecycle state reconciliation for %s failed before transition attempt %d: %s",
        node_name.c_str(), attempt, error.what());
      if (attempt == transition_retries_) {
        return false;
      }
      std::this_thread::sleep_for(transition_retry_interval_);
      continue;
    }

    bool response_success = false;
    try {
      response_success = node_map_[node_name]->change_state(transition, service_call_timeout_);
    } catch (const std::runtime_error & error) {
      RCLCPP_WARN(get_logger(),
        "Lifecycle transition response for %s was unavailable on attempt %d: %s; "
        "reconciling actual state before any retry",
        node_name.c_str(), attempt, error.what());
    }

    // A lost response is ambiguous: the transition may already have applied.
    // Query actual state before permitting another transition request.
    try {
      observed_state = node_map_[node_name]->get_state(service_call_timeout_);
      if (observed_state == target_state) {
        if (transition == Transition::TRANSITION_ACTIVATE) {
        ...
        return response_success || observed_state == target_state;
      }
      if (observed_state != State::PRIMARY_STATE_UNCONFIGURED &&
        observed_state != State::PRIMARY_STATE_INACTIVE &&
        observed_state != State::PRIMARY_STATE_ACTIVE)
      {
        RCLCPP_ERROR(get_logger(),
          "Unexpected reconciled lifecycle state %u for %s after transition %u",
          observed_state, node_name.c_str(), transition);
        return false;
      }
    } catch (const std::runtime_error & error) {
      RCLCPP_WARN(get_logger(),
        "Lifecycle state reconciliation for %s failed after attempt %d: %s",
        node_name.c_str(), attempt, error.what());
    }
    if (attempt < transition_retries_) {
      std::this_thread::sleep_for(transition_retry_interval_);
  ...
  RCLCPP_ERROR(get_logger(), "Failed to establish lifecycle state for node: %s", node_name.c_str());
  return false;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(phase4_lifecycle_manager_reconciled::LifecycleManager)
